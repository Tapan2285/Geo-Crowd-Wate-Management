      ...
      <div id="settings" style={{ marginBottom: '4rem' }}>
        ...
        <div className="card glass" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', borderBottom: '1px solid var(--border)', paddingBottom: '1.5rem' }}>
            <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '2rem', fontWeight: 'bold' }}>
              {userData ? userData.name?.charAt(0).toUpperCase() : 'M'}
            </div>
            <div>
              <h2 style={{ margin: '0 0 0.25rem 0', color: 'var(--text-main)' }}>
                {userData ? userData.name : 'Municipality Office'}
              </h2>
              <p style={{ margin: 0, color: 'var(--text-muted)' }}>
                {userData ? userData.email : 'municipality@test.com'}
              </p>
              <span className="badge badge-info" style={{ marginTop: '0.5rem', display: 'inline-block' }}>Municipality Account</span>
            </div>
          </div>
